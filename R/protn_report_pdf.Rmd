...
```{r differentiated_protein, echo=FALSE, results='asis', eval=(length(db_execution$differential_results)>0 & !is.null(db_execution$protein_differential_barplot))}
cat("#### Number of differentially expressed proteins for each comparison \n")
cat("\n \n")
db_execution$protein_differential_barplot
```

```{r differentiated_peptide, echo=FALSE, results='asis', eval=(length(db_execution$differential_results)>0 & !is.null(db_execution$peptide_differential_barplot))}
...
for(comp in names(db_execution$protein_vulcano)){
  cat(paste0("##### ",comp," \n \n"))
  htmlwidgets::saveWidget(widget = db_execution$protein_vulcano[[comp]], file = paste0("prot_",comp,".html"))
  webshot2::webshot(url = paste0("prot_",comp,".html"), file = paste0("prot_",comp,".png"), delay = 1, zoom = 4)
  cat(" \n \n")  
}
cat(" \n \n")
```
